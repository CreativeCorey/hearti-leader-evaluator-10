export const ja = {
  ...
  report: {
    header: {
      title: "21世紀のリーダーの皆様へ：",
      content: "あなたは過去ではなく、未来を代表しています。昨日の産業時代のために設計された能力に焦点を当てることで、リーダーシップスキルを開発するよう奨励されてきた可能性があります—今日に関連するものはもちろん、明日に関連するものはなおさらです。\n\n私たちは、明日の職場に必要な本質的なリーダーシップスキルを特定したので理解しています。認知心理学とポジティブ心理学、組織設計、パフォーマンス管理における最近の画期的な発見を研究しました。また、数十の業界の現代的なリーダーたちとも話しました。\n\nこのプロセスを通じて、21世紀の職場に必要な中核的な能力は、変革的リーダーがより良い結果を奨励するために意図的に使用する特性であることを理解しました。それらは謙遜、共感、責任感、回復力、透明性、包容性です。これらの能力を活用するリーダーは、イノベーション、創造性、ポジティブな従業員体験の道を築くでしょう。私たちの研究はまた、これらの特性を持つリーダーを雇用し育成する企業が、従来のリーダーシップ行動を持つリーダーを雇用し育成する企業よりも優れたパフォーマンスを示すことを明らかにしました。"
    },
    dimension: {
      analysisTitle: "HEARTI次元分析",
      analysisDescription: "以下の各次元は、あなたのスコア、能力レベル、継続的な発展のためのガイダンスを示しています。"
    },
    footer: {
      title: "世界を変える：",
      content: "このHEARTI:Leaderアセスメントレポートでは、この新しい労働世界でのリーダーシップを進化させるために必要なフレームワーク、ツール、リソースを提供しました—残りはあなた次第です。私たちの世界を前進させるために、毎日装備され、意欲的に目覚めることを願っています。\n\nHEARTI:Leaderになる方法や世界クラスの職場を育む方法についてのより多くの洞察と情報が必要な場合は、PrismWorkにお問い合わせください。HEARTI:Leaderの視点でチームを結束させるチームダイナミクスと開発ワークショップがあります。"
    }
  }
};
